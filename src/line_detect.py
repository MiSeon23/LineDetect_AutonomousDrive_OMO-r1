"""
line_detect_rev4.cpp
>> Get the error between right way and real way
""" 
...
def pipeline(img):
    ...
    if lines_raw is None :
        ...
        if left_x_start and right_x_start:
            ...
            mid_err_list = [round(mid_err), int(height/3), round(mid_err), int(height/6)]
            result_image = draw_lines(mid_line_image,
                [[mid_err_list]], color=[0,255,0], thickness=3
            )
            result_image = cv2.line(result_image, (int(width/2), int(height/3)), (int(width/2), int(height/6)), color=[0,0,255], thickness=3)
            result_image = cv2.line(result_image, (int(width/2), int(height/4)), (round(mid_err), int(height/4)), color=[0,255,0], thickness=3)
            err = width/2 - mid_err
